...
// SIMPLE WITHDRAWAL LOGIC - 5% FEE ON ALL WITHDRAWALS
// NO CAPITAL LOCKING - ALL FUNDS ARE WITHDRAWABLE
// ============================================
...
class WalletService {
  ...
  async getFiatBalance(userId) {
    try {
      ...
      // Calculate total deposits for reference
      ...
      return {
        ...
        availableProfit: totalTradingProfit,
        withdrawableBalance: wallet.balance, // ALL funds are withdrawable
  ...
  async withdrawFiat(userId, amountUGX, method, metadata = {}) {
    try {
      ...
      if (wallet.balance < amountUGX) {
        throw new Error(`Insufficient balance. Available: UGX ${wallet.balance.toLocaleString()}`);
      }
      
      const feeUGX = amountUGX * WITHDRAWAL_FEE;
      const netAmountUGX = amountUGX - feeUGX;
      
      console.log(`💸 Withdrawal Request:`);
      console.log(`   Amount: UGX ${amountUGX.toLocaleString()}`);
      console.log(`   Fee: ${WITHDRAWAL_FEE * 100}% (UGX ${feeUGX.toLocaleString()})`);
      console.log(`   Net Amount: UGX ${netAmountUGX.toLocaleString()}`);
      console.log(`   Balance: UGX ${wallet.balance.toLocaleString()}`);
      ...
      const transaction = {
        ...
        balanceBefore: wallet.balance,
        balanceAfter: wallet.balance - amountUGX,
        description: `Withdrawal via ${method} (${WITHDRAWAL_FEE * 100}% fee applied)`,
      ...
      console.log(`   New Balance: UGX ${wallet.balance.toLocaleString()}`);
      console.log(`   Total Fees Collected: UGX ${wallet.totalFeesCollected.toLocaleString()}`);
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          dailyTradingProfit: dailyTradingProfit,
