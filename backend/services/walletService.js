...
class WalletService {
  ...
  async creditCryptoWallet(userId, cryptoAmount, cryptoCurrency, purchaseRef) {
    try {
      ...
      // Check if blockchainService is available
      let blockchainService = null;
      try {
        const module = await import('./blockchainService.js');
        blockchainService = module.default;
      } catch (e) {
        console.log('?? Blockchain service not available, using simulated wallet');
      }
